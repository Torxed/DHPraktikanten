...
class httplib():
	...
	def navigate(self):
		...
		if self.htmldata['type'] == 'GET':
			...
			if not postdata:
				log('Problem formatting the POST data','CCO')
				sys.stdout.flush()
				return None
...
class Pages():
	...
	def dologin(self):
		return {
				...
				'target' : '/login.htm',
				'type' : 'POST',
				'form' : {'signin_username' : core['cco']['user'],
							'signin_password' : core['cco']['pass'],
							'hash' : '',
							'button' : 'Logga in'},
